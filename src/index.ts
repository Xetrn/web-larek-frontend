...
const productBasketView: ProductBasketView = new ProductBasketView(events);
...
events.on('product-view: click', (product: IProduct): void => {
	...
	modalView.render({
		isOpen: true,
		content: new ModalProductView(events).render({ item: product, isInBasket: isInBasket }),
	});
});
...
function basketRender() {
	...
	const itemsView = products.map((product, index) => {
		const view = new ProductBasketView(events).render({item: product, index: index + 1})
		return view
	})

	const content: HTMLDivElement = basketView.render({items: products, itemsView: itemsView})

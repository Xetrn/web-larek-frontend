...
basketIcon.addEventListener('click', () => {
	...
	const basketContent = new BasketModalView(basketElement);
	...
	basketContent.onOrderSubmit = () => eventEmitter.emit('basket:completed');
});
...
eventEmitter.on('basket:itemAdded', (e: { item: IItem }) =>
	basket.addItem(e.item)
);
eventEmitter.on('basket:itemRemoved', (e: { item: IItem }) =>
	basket.removeItem(e.item)
);

eventEmitter.on(
	'payment:completed',
	(e: { payment: string; address: string }) => basket.populateOrderData(e)
);
...
basket.onSave = async (saveData: any) => {
	try {
		...
		const successContent = new SuccessOrderModalView(successElement);

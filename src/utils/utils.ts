...
export function getNameOfClassCategory(name: string): string {
	const nameOfClass = 'card__category';
	switch (name) {
		case 'софт-скил':
			return nameOfClass + '_soft';
		case 'другое':
			return nameOfClass + '_other';
		case 'дополнительное':
			return nameOfClass + '_additional';
		case 'кнопка':
			return nameOfClass + '_button';
		default:
			return nameOfClass + '_hard';
	}
}

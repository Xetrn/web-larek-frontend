...
- ### emit
Инициирует событие, передавая подписчикам данные.
- ### onAll
Подписка на все события.
- ### offAll
Удаление всех подписчиков.
- ### trigger
Создает функцию, которая вызывает событие с предустановленным контекстом.
...
Вызвать событие и передать данные.

- trigger<T>(event: string, context?: Partial<T>): (data: T) => void
Создать функцию-генератор для события.

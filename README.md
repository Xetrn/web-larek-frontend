...
Хочется подметить, что в конструкторе класса `CatalogView` принимается `protected itemView: IView`, что делает класс более гибким и даёт возможность менять способ отрисовки карточек, лишь бы он соответствовал интерфейсу `IView`.
...
После наполнения корзины товарами, пользователь может оформить заказ, для этого необходимо в форме указать свои данные в этом нам поможет интерфейс `IUserData`:
